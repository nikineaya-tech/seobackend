...
} = require('./competitor-evidence-engine');
...
function productCompatibility(reportType, payload = {}, issues = []) {
  ...
  const family = text(
    payload.productUnderstanding?.productSemantics?.productFamily ||
    payload.productSemantics?.productFamily ||
    '',
    80
  );
  if (family === 'beauty_skin') {
    const matches = visibleText.match(/peau|skin|visage|hygiene|hygiène|aspiration|suction|embout|patch|institut|salon|routine|irritation|acn[eé]|blackhead|points noirs|بشرة|وجه|نظافة|شفط|الرؤوس السوداء|حساسة|صالون|روتين/gi) || [];
    if (new Set(matches.map(item => item.toLowerCase())).size < 3) {
      addIssue(issues, 'high', 'PRODUCT_SPECIFICITY_LOW', 'Les personas beauté ne contiennent pas assez de preuves, risques ou critères propres au soin visage.');
    }
  }
  if (family === 'education') {
    const matches = visibleText.match(/module|exercice|cours|formation|apprenant|support|livrable|méthode|methode|paiement|cod|fournisseur|whatsapp|instagram|تكوين|درس|تمرين|منهج|متعلم|دعم|مورد|الدفع/gi) || [];
    if (new Set(matches.map(item => item.toLowerCase())).size < 3) {
      addIssue(issues, 'high', 'PRODUCT_SPECIFICITY_LOW', 'Les personas formation ne contiennent pas assez de preuves, livrables ou contexte d’apprentissage spécifiques.');
    }
  }
}
...
function competitorClaimQuality(payload = {}, context = {}, issues = []) {
  if (context.reportType && context.reportType !== 'competitors') return;
  const competitors = asArray(payload.competitors || payload.top10Competitors || payload.enrichedCompetitors);
  if (!competitors.length && context.reportType !== 'competitors') return;
  const registry = payload.evidenceRegistry || createEvidenceRegistry({ competitors });
  const claimAudit = validateCompetitorClaims(payload, registry, {
    ...context,
    userSiteAudited: Boolean(payload.userSiteAudited || payload.userBenchmark?.url || context.userSiteAudited)
  });
  claimAudit.issues.forEach(issue => {
    addIssue(issues, issue.severity || 'medium', issue.code, issue.message, issue.path || 'competitors');
  });
}
...
function auditMarketingResponse(reportType, payload = {}, context = {}) {
  ...
  competitorClaimQuality(payload, { ...context, reportType }, issues);
