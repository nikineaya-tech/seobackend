...
function normalizedGeoCode(geo = '') {
  const text = normalize(geo);
  if (/libya|libye|ليبيا/.test(text)) return 'ly';
  if (/morocco|maroc|المغرب/.test(text)) return 'ma';
  if (/tunisia|tunisie|تونس/.test(text)) return 'tn';
  if (/algeria|alg[eé]rie|الجزائر/.test(text)) return 'dz';
  return '';
}

function competitorIsLocal(competitor = {}, geo = '') {
  if (competitor.geoConfirmed === true && /LOCAL_CONFIRMED|LOCAL_PROBABLE/i.test(String(competitor.geoTier || ''))) return true;
  const code = normalizedGeoCode(geo);
  const raw = normalize([competitor.domain, competitor.displayed_link, competitor.url, competitor.link, competitor.country, competitor.geoTier].filter(Boolean).join(' '));
  return Boolean(code && new RegExp('(?:\\.|^)' + code + '(?:\\.|$)').test(raw)) || (code === 'ly' && /libya|libye|ليبيا/.test(raw));
}

function selectRelevantCompetitor(competitors = [], geo = '') {
  const list = Array.isArray(competitors) ? competitors.filter(Boolean) : [];
  return list.find(item => competitorIsLocal(item, geo)) || list[0] || {};
}
  ...
  const scored = segments.map(segment => {
    ...
    return {
      ...
      scoreProvenance: {
        ...
        observedInputs: {
          competitorCount,
          localCompetitorCount: (competitorData?.top10Competitors || competitorData?.competitors || []).filter(item => competitorIsLocal(item, competitorData?.geoSourceAudit?.target || competitorData?.geo || '')).length,
          sourceCount,
          budgetProvided: Boolean(String(budget || '').trim())
        },
        derivedInputs: ['segment validation basis', 'keyword-like text signals', 'budget tier'],
        formula: 'weighted productFit + problemIntensity + purchaseIntent + reachability + differentiationPotential + evidenceStrength + commercialPotential - risk',
        warning: 'This is a prioritization heuristic, not a measured market probability.'
      }
    };
...
function buildPositioningObjects(targets = [], { query = '', geo = '', competitorData = {}, lang = 'fr' } = {}) {
  ...
  const leader = selectRelevantCompetitor(competitorData?.top10Competitors || competitorData?.competitors || [], geo);
