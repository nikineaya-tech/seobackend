...
function familyAngleTemplates(family = 'general_offer', lang = 'fr') {
  ...
  const familyBoost = {
    ...
    beauty_skin: {
      ...
      local_speed: { proof: { fr: 'villes livrées, délai annoncé et canal de support', en: 'served city list, announced delivery window and support channel', ar: 'منطقة التوصيل ومدة التسليم والتواصل المحلي' } },
...
function sanitizePersonaForDisplay(persona = {}, { query = '', market = '', lang = 'fr' } = {}) {
  ...
  const visibleTriggers = cleanTriggerArray(localizeDetailArray(details.buyingTriggers, 5), { max: 5, query, market });
...
function buildAngleDrivenStpModel({ query = '', geo = '', lang = 'fr', segments = [], personaCards = [], competitorData = {}, beachheadMarket = {}, budget = '' } = {}) {
  ...
  if (!angles.length) {
    angles = ['proof_outcome', 'expertise_method', 'security_trust']
      ...
      .map(angle => ({ ...angle, score: scoreMarketingAngle(angle, evidenceCatalog, { query: subjectQuery, budget }), opportunityScore: scoreMarketingAngle(angle, evidenceCatalog, { query: subjectQuery, budget }) }));
  }
