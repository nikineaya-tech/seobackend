...
function scoreMarketingAngle(angle = {}, evidence = {}, context = {}) {
  ...
  const score = 48 + Math.min(22, evidenceCount * 4) + leanBoost + solarBoost + categoryBoost + (normalized.type === 'security_trust' ? 7 : 0);
...
function buildAngleDrivenStpModel({ query = '', geo = '', lang = 'fr', segments = [], personaCards = [], competitorData = {}, beachheadMarket = {}, budget = '' } = {}) {
  ...
  angles = angles.map(angle => {
    ...
    scored.score = scoreMarketingAngle(scored, evidenceCatalog, { query: subjectQuery, budget, productSemantics });
  ...
  if (!angles.length) {
    angles = ['proof_outcome', 'expertise_method', 'security_trust']
      ...
      .map(angle => ({ ...angle, score: scoreMarketingAngle(angle, evidenceCatalog, { query: subjectQuery, budget, productSemantics }), opportunityScore: scoreMarketingAngle(angle, evidenceCatalog, { query: subjectQuery, budget, productSemantics }) }));
  }
