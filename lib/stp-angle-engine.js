...
function angleForPersonaDisplay(angle = {}) {
  ...
  return {
    ...
    proofNeeded: cleanDetailArray(angle.proofNeeded, { max: 5, query: angle.context || angle.jobToBeDone, market: angle.context }),
    hooks: cleanDetailArray(angle.hooks, { max: 4, query: angle.context || angle.jobToBeDone, market: angle.context }),
    channels: cleanChannelArray(angle.channels, 6),
    ...
    hookExamples: cleanDetailArray(angle.hookExamples, { max: 4, query: angle.context || angle.jobToBeDone, market: angle.context }),
    antiHallucinationChecks: cleanDetailArray(angle.antiHallucinationChecks, { max: 5, query: angle.context || angle.jobToBeDone, market: angle.context }),
...
function detectProductFamily(query = '') {
  ...
  if (/blackhead|comedon|points noirs|رؤوس سوداء|الرؤوس السوداء|beauty|skin|peau|بشرة/.test(text)) return 'beauty_skin';
...
function buildPersonaAttackBlueprint(persona = {}, angle = {}, { lang = 'fr', query = '', market = '' } = {}) {
  ...
  const need = safeText(details.primaryJobToBeDone || details.need || persona.summary || angle.jobToBeDone || '', 220);
...
function sanitizePersonaForDisplay(persona = {}, { query = '', market = '', lang = 'fr' } = {}) {
  ...
  return {
    ...persona,
    primaryAngle: angleForPersonaDisplay(persona.primaryAngle),
    secondaryAngles: (persona.secondaryAngles || []).map(angleForPersonaDisplay).filter(Boolean).slice(0, 2),
...
function buildAngleDrivenStpModel({ query = '', geo = '', lang = 'fr', segments = [], personaCards = [], competitorData = {}, beachheadMarket = {}, budget = '' } = {}) {
  ...
  const enrichedPersonas = mapPersonasToAngles(personaCards, angles).map((persona, index) => {
    ...
    const personaAttack = buildPersonaAttackBlueprint(persona, angle || {}, { lang, query, market: geo });
    ...
    return {
      ...
      details: {
        ...
        socialPlan: {
          ...
          contentAngles: safeArray([personaAttack.attackAngle, ...(personaAttack.hookExamples || []), angle?.promise, ...(angle?.hooks || []), ...safeArray((details.socialPlan || {}).contentAngles, 4)], 6)
        },
        stp: {
          ...(details.stp || {}),
  ...
  const sanitizedPersonas = enrichedPersonas.map(persona => sanitizePersonaForDisplay(persona, { query, market: geo, lang }));
