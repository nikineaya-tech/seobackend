...
function classifyProductSemantics({ query = '', geo = '', segments = [], competitorData = {} } = {}) {
  const allText = [
    query,
    geo,
...
function angleForPersonaDisplay(angle = {}, { lang = 'fr', query = '', market = '' } = {}) {
  ...
  return {
    ...
    trigger: angle.angleType === 'local_market_fit' ? angle.trigger : (localizedTrigger(angle.type, lang) || angle.trigger),
...
function sanitizePersonaForDisplay(persona = {}, { query = '', market = '', lang = 'fr' } = {}) {
  ...
  const cleanWant = cleanVisibleText(details.wantStatement || details.primaryJobToBeDone, { query, market, minScore: 20 });
  ...
  return {
    ...
    details: {
      ...details,
      repetitionScore,
      primaryJobToBeDone: adjustedJob || details.primaryJobToBeDone || cleanSummary,
...
function buildAngleDrivenStpModel({ query = '', geo = '', lang = 'fr', segments = [], personaCards = [], competitorData = {}, beachheadMarket = {}, budget = '', objective = '', resources = '', price = '' } = {}) {
  const subjectQuery = localizeSubjectForLang(query, lang);
  const subjectGeo = localizeMarketForLang(geo, lang);
  const productSemantics = classifyProductSemantics({ query: subjectQuery, geo: subjectGeo, segments, competitorData });
