...
function buildPersonaAttackBlueprint(persona = {}, angle = {}, { lang = 'fr', query = '', market = '' } = {}) {
  ...
  if (lang === 'ar') {
    ...
    personaSummary = `${name ? `${name}: ` : ''}هذه الشخصية تبحث عن ${subject} بزاوية "${promise}" وتحتاج إلى ${proof || 'دليل قابل للتحقق'} قبل القرار.`;
  } else if (lang === 'en') {
    ...
    personaSummary = `${name ? `${name}: ` : ''}Looks for ${subject} through the "${promise}" angle and needs ${proof || 'verifiable proof'} before acting.`;
  } else {
    ...
    personaSummary = `${name ? `${name} : ` : ''}Cherche ${subject} avec l'angle "${promise}" et veut voir ${proof || 'une preuve verifiable'} avant d'agir.`;
  }
  const formula = [
    name || 'Persona',
    need,
...
function sanitizePersonaForDisplay(persona = {}, { query = '', market = '', lang = 'fr' } = {}) {
  ...
  return {
    ...
    details: {
      ...details,
      primaryJobToBeDone: cleanWant || details.primaryJobToBeDone || cleanSummary,
