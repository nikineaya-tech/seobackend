...
function buildProjectedFrameworkWorkshops(report = {}, coverage = {}, lang = 'fr') {
  ...
  const answerFor = (key, label, index) => {
    const ar = {
      swot: [
        'ابدأ بالقوة التي يمكن إثباتها في صفحة البيع: سعر، دليل، ضمان، توصيل أو نتيجة مرئية.',
        'اعتبر الضعف الأول هو كل نقطة تجعل القرار غير واضح: السعر، الدليل، طريقة الاستخدام أو المخاطر.',
        'حوّل النقص المرصود عند المنافسين إلى اختبار رسالة أو صفحة قرار أو دليل قبل/بعد.',
        'عامل الخطر كاعتراض يجب كسره بدليل واضح وليس بوعد عام.'
      ],
      blue_ocean: [
        'احذف الكلام العام الذي لا يساعد العميل على معرفة لماذا يشتري الآن.',
        'قلل الميزات الثانوية إذا كانت تزاحم الدليل والسعر وطريقة الشراء.',
        'ارفع الوضوح حول النتيجة، الاستخدام، الثقة، والسعر قبل إضافة وعود جديدة.',
        'ابتكر زاوية قرار بسيطة: مقارنة واضحة أو دليل استخدام أو ضمان قابل للتحقق.'
      ],
      porter: [
        'إذا تشابهت العروض، فاربح بالوضوح والدليل لا بكثرة الكلام.',
        'اكتب البدائل التي يمكن أن يستخدمها العميل ثم بيّن لماذا عرضك أسهل أو أقل مخاطرة.',
        'إذا لم توجد أدلة موردين، ابقها غير مؤكدة واطلب تحقق sourcing منفصل.',
        'قوة العميل تأتي من المقارنة؛ أعطه معيار اختيار واضحا قبل أن يبحث عند غيرك.'
      ],
      jtbd_kano_aarrr: [
        'صغ المهمة كفعل عملي يريد العميل إنجازه، وليس كاسم المنتج فقط.',
        'ابحث عن تفصيل صغير يجعل العرض يشعره أنه مناسب له تحديدا.',
        'اختبر أين يسقط العميل في المسار: البحث، الفهم، الثقة، الطلب أو المتابعة.'
      ]
    };
    const en = {
      swot: [
        'Start with the strength that can be proven on the page: price, proof, guarantee, delivery or visible result.',
        'Treat the first weakness as anything that makes the decision unclear: price, proof, usage or risk.',
        'Turn a competitor gap into a message test, decision page or before/after proof.',
        'Treat each threat as an objection to answer with proof, not a broad promise.'
      ],
      blue_ocean: [
        'Remove generic wording that does not help the buyer know why to buy now.',
        'Reduce secondary features when they hide proof, price and purchase clarity.',
        'Raise clarity around result, usage, trust and price before adding new promises.',
        'Create a simple decision angle: clear comparison, usage proof or verifiable guarantee.'
      ],
      porter: [
        'If offers look similar, win through clarity and proof rather than more claims.',
        'List the substitutes buyers can use, then show why your offer is easier or lower risk.',
        'If supplier evidence is missing, keep it unknown and require a separate sourcing check.',
        'Buyer power comes from comparison; give buyers a clear choice criterion first.'
      ],
      jtbd_kano_aarrr: [
        'Write the job as a practical action the buyer wants done, not only the product name.',
        'Look for one small detail that makes the offer feel specifically relevant.',
        'Test where the buyer drops: search, understanding, trust, order or follow-up.'
      ]
    };
    const fr = {
      swot: [
        'Commence par la force prouvable sur la page: prix, preuve, garantie, livraison ou résultat visible.',
        'Traite comme faiblesse tout ce qui rend la décision floue: prix, preuve, usage ou risque.',
        'Transforme le manque observé chez les concurrents en test de message, page décision ou preuve avant/après.',
        'Traite chaque menace comme une objection à casser par une preuve, pas par une promesse générale.'
      ],
      blue_ocean: [
        'Supprime les phrases générales qui n’aident pas le client à savoir pourquoi acheter maintenant.',
        'Réduis les features secondaires quand elles cachent la preuve, le prix et la clarté d’achat.',
        'Augmente la clarté sur résultat, usage, confiance et prix avant d’ajouter de nouvelles promesses.',
        'Crée un angle de décision simple: comparaison claire, preuve d’usage ou garantie vérifiable.'
      ],
      porter: [
        'Si les offres se ressemblent, gagne par la clarté et la preuve plutôt que par plus de promesses.',
        'Liste les substituts possibles, puis montre pourquoi ton offre est plus simple ou moins risquée.',
        'Sans preuve fournisseur, garde cette partie inconnue et demande une vérification sourcing séparée.',
        'Le client compare; donne-lui d’abord un critère de choix clair.'
      ],
      jtbd_kano_aarrr: [
        'Écris la tâche comme une action concrète que le client veut accomplir, pas seulement le nom du produit.',
        'Cherche le petit détail qui fait sentir que l’offre est faite pour lui.',
        'Teste où le client décroche: recherche, compréhension, confiance, commande ou suivi.'
      ]
    };
    const table = lang === 'ar' ? ar : lang === 'en' ? en : fr;
    return table[key]?.[index] || cleanText(label, 200);
  };
  const makeWorkshop = (key, block) => ({
    key,
    title: block.title,
    cards: asArray(block.cards).map(([label, guide, answer], index) => ({
      id: `${key}_${index + 1}`,
      label,
      question: guide,
      answer: answer || answerFor(key, label, index),
      guide,
