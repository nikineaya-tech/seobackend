...
function isCommentsReviewsEvidence(row = {}) {
  ...
  const customerPlatforms = /^(youtube|reddit|facebook|instagram|tiktok|x|twitter|trustpilot|google_reviews?|review|reviews)$/i;
  const customerClaim = /review|avis|comment|testimonial|rating|opinion|feedback|reaction|complaint|objection|question|faq|customer|client|تقييم|آراء|اراء|مراجعة|تعليق|شكوى|اعتراض|سؤال/i;
  ...
  return Boolean(
    ...
    (
      scope === 'CUSTOMER' ||
      customerPlatforms.test(platform) ||
      customerClaim.test(claim) ||
      textHint.test(text)
    )
  );
}
...
function buildMarketCoverage(input = {}, marketSignalModel = {}, commentsReviews = {}) {
  ...
  const readyChannels = channelDiagnostics.filter(item => String(item.status || '').toUpperCase() === 'READY' && Number(item.evidenceCount || 0) > 0);
  ...
  const missingCapabilities = uniq([
    customerPatterns.length ? null : 'Customer Voice réel insuffisant',
    commentsReviews.summary?.evidenceCount ? null : 'Commentaires et avis exploitables non collectés',
    socialPatterns.length ? null : 'Social Content Intelligence faible',
    offerPatterns.length ? null : 'Offer Intelligence comparée faible',
    hasPricing ? null : 'Pricing Intelligence non vérifiée',
    hasProof ? null : 'Proof Intelligence faible',
    hasTrust ? null : 'Trust Intelligence faible',
    entities.substitutes ? null : 'Substituts absents',
    entities.suppliers ? null : 'Supplier Intelligence absente',
    metrics.datedEvidenceCount ? null : 'Temporal Intelligence absente',
    hasTraceableTopInsights ? null : 'Top Insights pas encore assez traçables'
  ], 14);
  ...
  const sections = [
    sectionCoverage({
      ...
      why: hasTraceableTopInsights ? 'Insights reliés à des signaux et preuves.' : 'Les insights existent peu ou manquent encore de trace signal/evidence.',
      ...
      next: 'Croiser Customer Voice, offres et concurrents avant recommandation.'
    }),
    sectionCoverage({
      key: 'customer_voice',
      label: 'Customer Voice réel',
      status: customerPatterns.length ? 'READY' : 'MISSING',
      why: customerPatterns.length ? 'Pains, objections ou critères issus de preuves.' : 'Aucun pattern client quantifié depuis des sources traçables.',
      evidenceCount: customerPatterns.reduce((sum, item) => sum + evidenceIdsOf(item).length, 0),
      signalCount: customerPatterns.length,
      next: 'Collecter avis, commentaires, questions et plaintes par plateforme.'
    }),
    sectionCoverage({
      key: 'comments_reviews',
      label: 'Commentaires et avis',
      status: commentsReviews.summary?.evidenceCount ? 'READY' : 'MISSING',
      why: commentsReviews.summary?.evidenceCount ? 'Commentaires/avis observés seulement, sans sentiment inventé.' : 'Aucun avis/commentaire exploitable dans ce run.',
      evidenceCount: commentsReviews.summary?.evidenceCount || 0,
      signalCount: commentsReviews.summary?.patternCount || 0,
      next: 'Activer ou corriger les canaux sociaux/reviews indisponibles.'
    }),
    sectionCoverage({
      key: 'social_content',
      label: 'Social Content Intelligence',
      status: socialPatterns.length >= 2 && readyChannels.length >= 2 ? 'READY' : (socialPatterns.length || readyChannels.length ? 'WEAK' : 'MISSING'),
      why: socialPatterns.length ? 'Des formats ou messages sociaux sont présents, mais la diversité peut rester limitée.' : 'Pas assez de hooks, formats, CTA ou objections socialement observés.',
      evidenceCount: socialPatterns.reduce((sum, item) => sum + evidenceIdsOf(item).length, 0),
      signalCount: socialPatterns.length,
      next: 'Analyser hooks, formats, commentaires, CTA et objections par canal.'
    }),
    sectionCoverage({
      key: 'offer_intelligence',
      label: 'Offer Intelligence comparée',
      status: offerPatterns.length >= 3 ? 'READY' : (offerPatterns.length ? 'WEAK' : 'MISSING'),
      why: offerPatterns.length ? 'Patterns d’offre détectés avec preuves.' : 'Matrice prix, garantie, livraison, paiement et preuve absente.',
      evidenceCount: offerPatterns.reduce((sum, item) => sum + evidenceIdsOf(item).length, 0),
      signalCount: offerPatterns.length,
      next: 'Construire la matrice prix / preuve / garantie / livraison / paiement.'
    }),
    sectionCoverage({ key: 'pricing', label: 'Pricing Intelligence', status: hasPricing ? 'READY' : 'MISSING', why: hasPricing ? 'Prix ou signaux prix observés.' : 'Aucun prix confirmé ni fourchette vérifiable.', evidenceCount: offerPatterns.filter(pattern => cleanText(pattern.aspect).toLowerCase() === 'pricing').reduce((sum, item) => sum + evidenceIdsOf(item).length, 0), signalCount: offerPatterns.filter(pattern => cleanText(pattern.aspect).toLowerCase() === 'pricing').length, next: 'Ne pas transformer une promesse en prix ni un prix en promesse.' }),
    sectionCoverage({ key: 'proof_trust', label: 'Proof & Trust Intelligence', status: hasProof && hasTrust ? 'READY' : (hasProof || hasTrust ? 'WEAK' : 'MISSING'), why: hasProof || hasTrust ? 'Quelques preuves ou signaux de confiance existent.' : 'Avis, UGC, garantie, retour, paiement ou contact non vérifiés.', evidenceCount: offerPatterns.filter(pattern => /payment|fulfillment|risk_reversal|proof/i.test(cleanText(pattern.aspect))).reduce((sum, item) => sum + evidenceIdsOf(item).length, 0) + (commentsReviews.summary?.evidenceCount || 0), signalCount: offerPatterns.filter(pattern => /payment|fulfillment|risk_reversal|proof/i.test(cleanText(pattern.aspect))).length, next: 'Séparer preuve observée, inférence prudente et recommandation test.' }),
    sectionCoverage({ key: 'entity_geo', label: 'Geo & Entity Classification', status: (entities.directCompetitors + entities.localSellers + entities.regionalBenchmarks + entities.marketplaces) ? 'READY' : 'WEAK', why: 'Les entités doivent rester typées: direct, local, benchmark, marketplace, substitut ou fournisseur.', evidenceCount: metrics.evidenceCount, signalCount: entities.directCompetitors + entities.localSellers + entities.regionalBenchmarks + entities.marketplaces, next: 'Expliquer dans l’UI pourquoi un résultat est local probable ou benchmark.' }),
    sectionCoverage({ key: 'substitutes_suppliers_temporal', label: 'Substituts, fournisseurs, temporalité', status: entities.substitutes || entities.suppliers || metrics.datedEvidenceCount ? 'WEAK' : 'MISSING', why: 'Ces couches nécessitent des preuves spécialisées et datées.', evidenceCount: metrics.datedEvidenceCount, signalCount: entities.substitutes + entities.suppliers, next: 'Garder UNKNOWN lorsque fournisseur, substitut ou tendance ne sont pas prouvés.' })
  ];
  ...
  return {
    ...
    quality: {
      ...
      limitations: [
        'LOW or MEDIUM coverage means the report must stay cautious and avoid full-market claims.',
        'SERP visibility is not market share.',
        ...(missingCapabilities.length ? ['Some intelligence blocks are missing because no traceable evidence was collected for them.'] : [])
      ]
    }
  };
}
...
function buildCommentsReviews(input = {}, marketSignalModel = {}) {
  ...
  const uniqueEvidence = uniqObjects(evidenceRows, item => `${item.kind}|${item.sourceUrl}|${item.value}`, 24);
  const diagnostics = asArray(input.agentReachEvidence?.channelDiagnostics)
    .map(compactChannelDiagnostic)
    .filter(item => item.channel)
    .slice(0, 16);
  const patterns = uniqObjects([
    ...asArray(marketSignalModel.customerVoice?.patterns).map(compactCustomerVoicePattern),
  ...
  ].filter(pattern => evidenceIdsOf(pattern).length > 0), item => `${item.type || item.format}|${item.key}|${item.evidenceIds.join(',')}`, 12);
  ...
  return {
    ...
    quality: {
      ...
      limitations: [
        'This section only shows comments, reviews, questions or customer-voice evidence collected from traceable sources.',
        ...(uniqueEvidence.length ? [] : ['No exploitable comments or reviews were collected in this run. Do not infer customer sentiment from absence of evidence.']),
        ...(platforms.length >= 2 ? [] : ['Customer voice source diversity is still low until another independent platform confirms the same pattern.'])
      ]
    }
  };
}
