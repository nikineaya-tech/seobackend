...
function sourceUrlsOf(item = {}, limit = 5) {
  ...
  ].filter(url => !blockedCustomerVoiceUrl(url)), limit);
}

function blockedCustomerVoiceUrl(url = '') {
  const value = cleanText(url, 900).toLowerCase();
...
function loginWallOrAuthText(text = '') {
...
function isCustomerVoicePlatform(row = {}) {
  ...
  if (blockedCustomerVoiceUrl(url)) return false;
...
function isCommentsReviewsEvidence(row = {}) {
  ...
  const url = cleanText(row.sourceUrl || row.url, 600);
  ...
  if (blockedCustomerVoiceUrl(url) || loginWallOrAuthText(text)) return false;
