...
function isCustomerVoicePlatform(row = {}) {
  ...
  if (blockedCustomerVoiceUrl(`${row.sourceUrl || ''} ${row.url || ''}`)) return false;
...
function isCustomerVoiceRow(row = {}, context = {}) {
  ...
  if (blockedCustomerVoiceUrl(`${row.sourceUrl || ''} ${row.url || ''}`) || loginWallOrAuthText(`${row.title || ''} ${row.value || ''}`)) return false;
