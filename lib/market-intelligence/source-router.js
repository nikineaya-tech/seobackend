...
function localizedCountryName(country = '', lang = '') {
  const raw = cleanText(country, 70);
  const key = raw.toLowerCase().replace(/[^a-z\u0600-\u06ff]+/g, ' ').trim();
  const map = {
    libya: { ar: 'ليبيا', fr: 'Libye', en: 'Libya' },
    ly: { ar: 'ليبيا', fr: 'Libye', en: 'Libya' },
    morocco: { ar: 'المغرب', fr: 'Maroc', en: 'Morocco' },
    maroc: { ar: 'المغرب', fr: 'Maroc', en: 'Morocco' },
    ma: { ar: 'المغرب', fr: 'Maroc', en: 'Morocco' },
    tunisia: { ar: 'تونس', fr: 'Tunisie', en: 'Tunisia' },
    tunisie: { ar: 'تونس', fr: 'Tunisie', en: 'Tunisia' },
    tn: { ar: 'تونس', fr: 'Tunisie', en: 'Tunisia' },
    algeria: { ar: 'الجزائر', fr: 'Algérie', en: 'Algeria' },
    algerie: { ar: 'الجزائر', fr: 'Algérie', en: 'Algeria' },
    dz: { ar: 'الجزائر', fr: 'Algérie', en: 'Algeria' },
    egypt: { ar: 'مصر', fr: 'Égypte', en: 'Egypt' },
    egypte: { ar: 'مصر', fr: 'Égypte', en: 'Egypt' },
    eg: { ar: 'مصر', fr: 'Égypte', en: 'Egypt' }
  };
  const entry = map[key];
  if (!entry) return raw;
  if (/^ar/i.test(lang)) return entry.ar;
  if (/^fr/i.test(lang)) return entry.fr;
  return entry.en;
}

function compactMarketSearchQuery(query = '', country = '', lang = '') {
  const raw = cleanText(query, 260);
  const market = localizedCountryName(country, lang);
  const knownHeads = [
    ...
    /formation\s+e-?commerce(?:\s+en\s+ligne)?/i,
    /projecteur\s+solaire/i
  ];
  const detected = knownHeads.map(pattern => raw.match(pattern)?.[0]).find(Boolean);
  const compact = cleanText(detected || raw
    .replace(/[0-9٠-٩]+(?:[×x%]|(?:\s*(?:مرة|seconds?|secondes|ثوان|ثانية)))?/gi, ' ')
    .replace(/[،,.;؛:()[\]{}"“”'’`]+/g, ' ')
    .split(/\s+/)
    .filter(token => token.length > 1 && !/^(avec|with|pour|dans|مزود|ينصح|تناسب|مستويات|شحن|الضوء|الأزرق|مختلف|أنواع|ناعمة)$/i.test(token))
    .slice(0, 7)
    .join(' '), 80);
  if (!market || new RegExp(market.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i').test(compact)) return compact;
  return cleanText(`${compact} ${market}`, 100);
}

function buildQueryVariants({ query = '', country = '', lang = '' } = {}) {
  const q = compactMarketSearchQuery(query, country, lang);
  const c = localizedCountryName(country, lang);
  const countryAlreadyIncluded = c && new RegExp(c.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i').test(q);
  const suffix = c && !countryAlreadyIncluded ? ` ${c}` : '';
  const geoPart = suffix || '';
  const proofQueries = /^ar/i.test(lang)
    ? [
        `${q}${geoPart} آراء العملاء`,
        `${q}${geoPart} تقييمات`,
        `${q}${geoPart} مراجعات`,
        `${q}${geoPart} تعليقات`,
        `${q}${geoPart} تجربة`
    ...
    : /^en/i.test(lang)
      ? [
          `${q}${geoPart} reviews`,
          `${q}${geoPart} customer reviews`,
          `${q}${geoPart} comments`,
          `${q}${geoPart} testimonials`,
          `${q}${geoPart} experience`
        ]
      : [
          `${q}${geoPart} avis`,
          `${q}${geoPart} avis clients`,
          `${q}${geoPart} commentaires`,
          `${q}${geoPart} témoignages`,
          `${q}${geoPart} expérience`
        ];
  const variants = [
    q,
    `${q}${suffix}`,
  ...
  if (/^ar/i.test(lang)) {
    variants.push(`${q} مراجعات${suffix}`, `${q} آراء${suffix}`, `${q} تعليقات${suffix}`, `${q} سعر${suffix}`, `${q} مقارنة${suffix}`);
  }
  if (/^fr/i.test(lang)) {
    variants.push(`${q} avis${suffix}`, `${q} avis clients${suffix}`, `${q} commentaires${suffix}`, `${q} prix${suffix}`, `${q} comparatif${suffix}`);
  }
  return uniq(variants.map(item => cleanText(item.replace(/\s+/g, ' '), 140)), 16);
}
