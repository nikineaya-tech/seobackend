...
function domainDiversity(rows = []) {
  return new Set(rows.map(row => hostOf(row.sourceUrl || row.entityId)).filter(Boolean)).size;
}
...
function scoreInsight({ evidence = [], crossSourceCount = 2, relationStrength = 1, specificity = 1, confidence = 'LOW' } = {}) {
  ...
  const freshnessScore = asArray(evidence).some(row => row.publishedAt || row.observedAt || row.collectedAt) ? 4 : 0;
  const geoScore = asArray(evidence).some(row => /LOCAL|DIRECT|LY|Libya|Maroc|Morocco/i.test(`${row.scope} ${row.country} ${row.geoTier} ${row.value}`)) ? 3 : 0;
...
function scoreBreakdown({ evidence = [], crossSourceCount = 2, relationStrength = 1, specificity = 1 } = {}) {
  ...
  return {
    ...
    freshnessScore: rows.some(row => row.publishedAt || row.observedAt || row.collectedAt) ? 0.8 : 0.2,
    geoRelevanceScore: rows.some(row => /LOCAL|DIRECT|LY|Libya|Maroc|Morocco/i.test(`${row.scope} ${row.country} ${row.geoTier} ${row.value}`)) ? 0.8 : 0.35
  };
}
...
function detectPriceValueAsymmetry(ctx) {
  ...
  const highPriceProof = highPrice.filter(item => /verified review|before after|guarantee|warranty|case study|proof|تقييم موثق|قبل وبعد|ضمان|دليل/i.test(normalize(item.row.value)));
  const proofRows = patternRows(ctx.evidenceRows, /verified review|before after|guarantee|warranty|case study|proof|تقييم موثق|قبل وبعد|ضمان|دليل/);
...
function buildInsightDiscoveryEngine(input = {}) {
  ...
  const detected = [
    ...
    ...detectProofGaps(ctx),
    ...detectTrustGaps(ctx),
    ...detectPriceValueAsymmetry(ctx),
    ...detectSaturation(ctx),
    ...detectLocalGlobalAsymmetry(ctx),
    ...detectSocialContentGaps(ctx),
    ...detectSubstituteRisks(ctx),
    ...detectSupplierLocalGaps(ctx),
    ...detectTemporalOpportunities(ctx)
  ].filter(Boolean);
  const seen = new Set();
