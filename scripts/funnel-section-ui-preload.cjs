 ...
 *
 * Adds a thin client-side layer to enrich the existing Funnel "Sections et textes"
 * block with funnelSectionScanner data. It does not create a duplicate top-level
 * report section.
 */
...
(function () {
  ...
  function findExistingSectionsCard() {
    const cards = Array.from(document.querySelectorAll('#resultsFunnel .result-card, .result-card'));
    return cards.find(card => {
      const text = (card.querySelector('h3')?.textContent || card.textContent || '').toLowerCase();
      return text.includes('sections') || text.includes('textes') || text.includes('autopsie') || text.includes('microscope') || text.includes('أقسام') || text.includes('sections label');
    }) || null;
  }
  ...
  function mount(scanner) {
    ...
    if (title && title.parentNode) title.insertAdjacentHTML('afterend', html);
    ...
    const wrapped = function () {
      return originalFetch.apply(this, arguments).then(function (response) {
        try {
          ...
          if (/\/api\/(analyze-funnel|funnel)\b/i.test(url)) {
