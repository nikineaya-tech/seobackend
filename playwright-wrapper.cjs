...
// ═══════════════════════════════════════════════════════════════════
// PLAYWRIGHT WRAPPER — Provider routing avec fallback multi-niveaux
// Ordre: Browserless → Local → Scrape.do → Search fallback
// Détection env: RENDER | RAILWAY | LOCAL
// ═══════════════════════════════════════════════════════════════════
...
// ── ENV NORMALIZATION ─────────────────────────────────────────────
const ENV = {
    BROWSERLESS_API_TOKEN: process.env.BROWSERLESS_API_TOKEN || process.env.BROWSERLESS_TOKEN || '',
    SCRAPE_DO_TOKEN:       process.env.SCRAPE_DO_TOKEN || process.env.SCRAPEDOTOKEN || '',
    SERPAPI_KEY:           process.env.SERPAPI_KEY || '',
    SERPER_API_KEY:        process.env.SERPER_API_KEY || '',
    KEYWORDS_EVERYWHERE_KEY: process.env.KEYWORDS_EVERYWHERE_KEY || process.env.KE_API_KEY || '',
    // Playwright binary path (Railway / custom)
...
/**
 * Render.com : variable RENDER=true
 */
...
/**
 * Railway.app : variable RAILWAY_ENVIRONMENT est définie automatiquement
 */
function isRailwayEnv() {
...
/**
 * Environnement cloud générique (Render OU Railway)
 */
function isCloudEnv() {
    return isRenderEnv() || isRailwayEnv();
}

function isBrowserlessEnabled() { return !!ENV.BROWSERLESS_API_TOKEN; }
function isScrapeDoEnabled()    { return !!ENV.SCRAPE_DO_TOKEN; }
function isSerpApiEnabled()     { return !!ENV.SERPAPI_KEY; }
function isSerperEnabled()      { return !!ENV.SERPER_API_KEY; }
...
function getEmptyRemoteResult(provider = 'unknown', error = 'Provider unavailable') {
    return { provider, browser: null, context: null, page: null, html: '', error, close: async () => {} };
}

// ── ENV LOG (une fois au démarrage) ──────────────────────────────
(function logEnvDetection() {
    const env = isRenderEnv() ? 'RENDER' : isRailwayEnv() ? 'RAILWAY' : 'LOCAL';
    const browserless = isBrowserlessEnabled() ? '✅' : '❌';
    const scrapeDo    = isScrapeDoEnabled()    ? '✅' : '❌';
    console.log(`[PlaywrightWrapper] ENV=${env} | Browserless=${browserless} | ScrapeDo=${scrapeDo}`);
    if (ENV.PLAYWRIGHT_BROWSERS_PATH) {
        console.log(`[PlaywrightWrapper] PLAYWRIGHT_BROWSERS_PATH=${ENV.PLAYWRIGHT_BROWSERS_PATH}`);
    }
})();
...
async function preparePage(page, url, mode = 'unknown') {
    ...
    try {
        ...
        await cursor.moveTo({ x: 100 + Math.random() * 400, y: 100 + Math.random() * 300 }).catch(() => {});
        await page.evaluate(() => window.scrollBy({ top: 300, behavior: 'smooth' })).catch(() => {});
        await page.waitForTimeout(1500);
    } catch (gotoErr) {
        console.warn(`⚠️ [BROWSER:${mode}] goto timeout/blocage ignoré: ${gotoErr.message}`);
    }
    ...
    await page.waitForFunction(
        ...
        console.warn(`⚠️ [BROWSER:${mode}] Body introuvable, continuation...`);
    });
...
async function launchLocalBrowser(url) {
    ...
    // Priorité : PLAYWRIGHT_CHROMIUM_PATH > PLAYWRIGHT_BROWSERS_PATH > auto
    const launchOptions = {
        headless: true,
        args: [
            '--no-sandbox',
    ...
    if (ENV.PLAYWRIGHT_CHROMIUM_PATH) {
        launchOptions.executablePath = ENV.PLAYWRIGHT_CHROMIUM_PATH;
        console.log(`[PlaywrightWrapper] executablePath=${ENV.PLAYWRIGHT_CHROMIUM_PATH}`);
    }
...
async function launchBrowserless(url) {
    ...
    if (!wsEndpoint) {
        ...
        return getEmptyRemoteResult('browserless', 'BROWSERLESS_API_TOKEN missing');
    }
    ...
    try {
        ...
        context = await browser.newContext({
            userAgent: USER_AGENT, locale: 'fr-FR', timezoneId: 'Africa/Casablanca',
            viewport: { width: 1280, height: 800 }, ignoreHTTPSErrors: true,
            javaScriptEnabled: true, hasTouch: false,
        });
        await prepareContext(context);
        const page = await context.newPage();
        ...
        return {
            provider: 'browserless', browser, context, page, html: '',
            close: async () => {
                try { await context?.close().catch(() => {}); } catch (_) {}
                try { await browser?.close().catch(() => {}); } catch (_) {}
            },
...
async function launchScrapeDo(url) {
    ...
    try {
        ...
        const response = await axios.get(apiUrl, {
            timeout: 45000,
            headers: { 'User-Agent': USER_AGENT, 'Accept-Language': 'fr-FR,fr;q=0.9,en-US;q=0.8,en;q=0.7' },
        });
        ...
        if (!html || html.length < 500) throw new Error(`HTML trop court (${html?.length || 0} chars)`);
        console.log(`✅ [SCRAPE.DO] HTML récupéré : ${html.length} chars`);

        return { provider: 'scrape.do', browser: null, context: null, page: null, html, close: async () => {} };
    } catch (err) {
...
async function searchFallbackUrl(url) {
    ...
    try {
        if (isSerperEnabled()) {
            console.log(`🔎 [SERPER] Recherche fallback : ${domain}`);
            const res = await axios.post(
                'https://google.serper.dev/search',
                { q: `site:${domain}` },
                { timeout: 12000, headers: { 'X-API-KEY': ENV.SERPER_API_KEY, 'Content-Type': 'application/json' } }
            );
        ...
        if (isSerpApiEnabled()) {
            console.log(`🔎 [SERPAPI] Recherche fallback : ${domain}`);
            const res = await axios.get('https://serpapi.com/search.json', {
                timeout: 12000,
                params: { engine: 'google', q: `site:${domain}`, api_key: ENV.SERPAPI_KEY },
            });
...
// ── MAIN ENTRY ────────────────────────────────────────────────────
// Ordre de priorité:
//   RENDER  → Browserless → ScrapeDo → SearchFallback → vide
//   RAILWAY → Local (binaires auto-installés) → ScrapeDo → vide
//   LOCAL   → Browserless (si BROWSER_PROVIDER=browserless) → Local → ScrapeDo
// ─────────────────────────────────────────────────────────────────
async function launchPlaywright(url) {

    // ── RENDER: préfère Browserless (pas de binaires dispo) ───────
    if (isRenderEnv()) {
        const remote = await launchBrowserless(url);
        if (remote.page || remote.html) return remote;

        console.warn('⚠️ [WRAPPER:RENDER] Browserless indisponible → Scrape.do...');
        const scrapeDo = await launchScrapeDo(url);
        if (scrapeDo.html) return scrapeDo;

        const altUrl = await searchFallbackUrl(url);
        if (altUrl && altUrl !== url) {
            console.warn(`⚠️ [WRAPPER:RENDER] URL alternative : ${altUrl}`);
            const remoteAlt = await launchBrowserless(altUrl);
            if (remoteAlt.page || remoteAlt.html) return remoteAlt;
            const scrapeDoAlt = await launchScrapeDo(altUrl);
    ...
    // ── RAILWAY: binaires auto-installés via postinstall ─────────
    if (isRailwayEnv()) {
        console.log(`[PlaywrightWrapper] Mode RAILWAY → launchLocalBrowser`);

        // Si Browserless est configuré, on peut aussi l'essayer en premier
        if (isBrowserlessEnabled()) {
            const remote = await launchBrowserless(url);
            if (remote.page || remote.html) return remote;
            console.warn('⚠️ [WRAPPER:RAILWAY] Browserless failed → fallback local...');
        }

        const local = await launchLocalBrowser(url);
        if (local.page) return local;

        console.warn('⚠️ [WRAPPER:RAILWAY] Local failed → Scrape.do...');
        const scrapeDo = await launchScrapeDo(url);
        if (scrapeDo.html) return scrapeDo;

        return getEmptyRemoteResult('railway-fallback', 'Local browser and Scrape.do failed');
    }
    ...
    if (isBrowserlessEnabled() && process.env.BROWSER_PROVIDER === 'browserless') {
        const remote = await launchBrowserless(url);
        if (remote.page || remote.html) return remote;
        console.warn('⚠️ [WRAPPER:LOCAL] Browserless failed → fallback local...');
    }
...
async function isAvailable() {
    if (isRenderEnv())  return isBrowserlessEnabled() || isScrapeDoEnabled();
    if (isRailwayEnv()) return true; // binaires présents après postinstall
    return true;
}
